...
class Baseline(nn.Module):
    ...
    def Loss(self, prediction, target, validation=False, onlyRV=False):
        contributes = {
            f.__name__: f(prediction, target)
    ...
    def Metrics(self, pred_seg, gt_seg, pred_vendor=None, gt_vendor=None):
        metrics = {}
        for c, key in enumerate(["LV_", "MYO_", "RV_"], start=1):
            ref = np.copy(gt_seg)
            pred = np.copy(pred_seg)
            ref = np.where(ref != c, 0, 1)
            pred = np.where(pred != c, 0, 1)
            metrics[key + "dc"] = DC(pred, ref)  # Dice Coefficient
            metrics[key + "hd"] = HD(pred, ref)  # Hausdorff Distance

        if pred_vendor is not None and gt_vendor is not None:
            metrics["v_accuracy"] = accuracy_score(gt_vendor, pred_vendor)
            metrics["v_f1"] = f1_score(gt_vendor, pred_vendor, average="weighted")
            metrics["v_precision"] = precision_score(
                gt_vendor, pred_vendor, average="weighted"
            )
    ...
    def adjust_learning_rate(self):
        if self.alr_counter <= 1000:
            self.optimizer.param_groups[0]["lr"] = (
                self.args.lr * (1 - self.alr_counter / 1000) ** 0.9
            )
        self.alr_counter += 1


def CELoss(prediction, target, dann=False):
